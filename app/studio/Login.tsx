...
export function Login({ configured, mode = "local" }: { configured: boolean; mode?: "local" | "supabase" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === "supabase") {
        ...
        const response = await fetch("/api/studio/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
