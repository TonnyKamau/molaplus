...
export async function GET() { try { await requireEditor(); return Response.json({ media: listMedia() }, { headers: { "Cache-Control": "no-store" } }); } catch (error) { return apiError(error); } }
