...
export function GET() {
  ...
  const items = publishedPosts().sort((a, b) => b.date.localeCompare(a.date)).map((post) => `<item><title>${escape(post.title)}</title><link>${base}/blog/${post.slug}</link><guid>${base}/blog/${post.slug}</guid><description>${escape(post.excerpt)}</description><category>${escape(post.category)}</category><pubDate>${new Date(`${post.date}T09:00:00+03:00`).toUTCString()}</pubDate></item>`).join("");
